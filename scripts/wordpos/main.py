#!/usr/bin/env python3
"""
Main CLI entry point for Voynich manuscript word position analysis.
...
from .config import WordPosConfig
from .analyzer import analyze_page_positions
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description='Analyze character word-position preferences in Voynich manuscript',
...
Examples:
  ...
  python -m scripts.wordpos.main --input data/voynich-transcription.txt --output docs/output/wordpos/ --bigram-mode collapsed
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Minimum word length to include in analysis'
    )
...
def main():
    ...
    # Analyze positions for each page
    if args.verbose:
        print("\nAnalyzing word positions per page...")
    
    page_positions = {}
    for page in pages:
        pc = analyze_page_positions(
            page,
    ...
    if args.verbose:
        for name, result in results.items():
            print(f"  {name}: {result.page_count} pages, {result.word_count} words")
