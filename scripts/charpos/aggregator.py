"""
Aggregator for Voynich manuscript character position counts.
...
from .analyzer import CharPositionCounts, PageCharPositionCounts


@dataclass
class AggregationResult:
    """Result of aggregating character position counts."""
    name: str
    description: str
    page_count: int
    word_count: int
...
def aggregate_pages(
    pages: list[Page],
    page_positions: dict[str, PageCharPositionCounts],
    name: str,
    description: str,
    ...
    """
    Aggregate character position counts for pages matching a filter.
    """
    matching_pages = [p for p in pages if filter_func(p)]
    
    merged_counts = CharPositionCounts(
