/**
 * Main application module for Voynich physical page position visualizer.
...
const App = (function() {
  let state = {
    ...
    resolution: 'coarse',
    normalization: 'page',
    clipHeight: 100,  // Max height cells in raw mode
    clipWidth: 80,    // Max width cells in raw mode
  ...
  async function init() {
    elements = {
      ...
      clipHeight: document.getElementById('clip-height'),
      clipWidth: document.getElementById('clip-width'),
  ...
  function setupEventListeners() {
    ...
    if (elements.clipHeight) {
      elements.clipHeight.addEventListener('change', (e) => {
        state.clipHeight = parseInt(e.target.value, 10) || 100;
        renderHeatmap();
      });
    }
    if (elements.clipWidth) {
      elements.clipWidth.addEventListener('change', (e) => {
        state.clipWidth = parseInt(e.target.value, 10) || 80;
        renderHeatmap();
      });
    }
  ...
  function handleHover(info) {
    ...
    // For raw mode with absolute coordinates, show approximate pixel position
    if (info.isRawMode && info.isAbsolute && info.pixelX !== null) {
      positionHtml = `
        <span class="cell-position">
          <span class="pixel-position">~(${info.pixelX}px, ${info.pixelY}px)</span>
        </span>
      `;
    } else {
      positionHtml = `
  ...
  function exportImage() {
    ...
    link.download = `voynich-physpagepos-${state.selectedChar}-${state.normalization}-${state.resolution}-${state.currentAggregation}.png`;
    link.href = dataUrl;
    link.click();
  }
