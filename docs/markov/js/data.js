/**
 * Data loading module for Voynich transition visualizer.
 * Handles fetching and caching of manifest and aggregation JSON files.
 */

const DataLoader = (function() {
  const cache = new Map();
  const BASE_PATH = '../output/markov/aggregated';
   ...
   * @returns {Promise<Object>} Aggregation data with transitions, probabilities, etc.
   */
  async function loadAggregation(name) {
    ...
    try {
      ...
      // Process the data to create lookup structures
   ...
   * @returns {Object} Matrix with lookup by [from][to]
   */
  function buildTransitionMatrix(data) {
    ...
    // Initialize empty rows for all characters in charset
   ...
   * @param {string} from - Source character
   * @param {string} to - Target character
   * @param {string} type - 'counts' or 'probabilities'
   * @returns {number} Transition value or 0
   */
   ...
   * @param {string[]} names - Array of aggregation names
   * @returns {Promise<Object[]>} Array of loaded aggregation data
   */
   ...
   * Returns a new data object with diff values (A - B) for each cell.
   * @param {Object} dataA - First aggregation data
   * @param {Object} dataB - Second aggregation data
   * @returns {Object} Diff data with diffMatrix and metadata
   */
   ...
   * @param {Object} diffMatrix - Diff matrix object
   * @param {string} from - Source character
   * @param {string} to - Target character
   * @returns {number} Diff value (can be negative)
   */
